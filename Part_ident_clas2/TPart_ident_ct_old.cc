...
Double_t TPart_ident_ct::ein( Int_t k) {
  if( GetNRows("EVNT")>0 &&  GetNRows("ECPB")>0){
    ...
    if(!evnt) p_ein=good1;
      ...
      ecpb = (TECPBClass*) GetBankRow("ECPB",ec_stat);
      if(!ecpb) p_ein=good1;
      else if (ecpb->Ein ==0 && ecpb->Innstr>0){  
...
Double_t TPart_ident_ct::eout( Int_t k) {
  if( GetNRows("EVNT")>0 &&  GetNRows("ECPB")>0){
    ...
    if(!evnt) p_eout=good1;
      ...
      else if (ecpb->Eout ==0 && ecpb->Outstr>0) {   
	    Int_t stat=Correct_ECPB_bank(ecpb);
	    p_eout=ecpb->Eout;
      }
      else
...
Int_t TPart_ident_ct::id_mctk() {
  Int_t p_id_mc = 0;

  if ( GetHEADER()->NRun == 10 &&  GetNRows("MCTK")>0) {  // simulation runs have runnnumber =10
				// , data files different
    mctk = (TMCTKClass*) GetBankRow("MCTK",0);
    p_id_mc = mctk->Getparent() ;
  }
...
Double_t TPart_ident_ct::px_mctk( Int_t k) {
  if ( GetHEADER()->NRun == 10 &&  GetNRows("MCTK")>0) {  // simulation runs have runnnumber =10
				// , data files different
   for(Int_t i = 0 ; i < GetNRows("MCTK") ; i++) {
    mctk = (TMCTKClass*) GetBankRow("MCTK",i);
    if(!mctk) p_px=good1;
    else {
     if (mctk->Getid() == k) {
      p_px= (double)(mctk->Getcx() * mctk->Getpmom()) ;
     }
    }
   }
  }
...
Double_t TPart_ident_ct::py_mctk( Int_t k) {
  if ( GetHEADER()->NRun == 10 &&  GetNRows("MCTK")>0) {  // simulation runs have runnnumber =10
				// , data files different
   for(Int_t i = 0 ; i < GetNRows("MCTK") ; i++) {
    mctk = (TMCTKClass*) GetBankRow("MCTK",i);
    if(!mctk) p_py=good1;
     ...
     if (mctk->Getid() == k) {
      p_py= (double)(mctk->Getcy() * mctk->Getpmom()) ;
     }
    }
   }
  }
...
Double_t TPart_ident_ct::pz_mctk( Int_t k) {
  if ( GetHEADER()->NRun == 10 &&  GetNRows("MCTK")>0) {  // simulation runs have runnnumber =10
				// , data files different
   for(Int_t i = 0 ; i < GetNRows("MCTK") ; i++) {
    mctk = (TMCTKClass*) GetBankRow("MCTK",i);
    if(!mctk) p_pz=good1;
     ...
     if (mctk->Getid() == k) {
      p_pz= (double)(mctk->Getcz() * mctk->Getpmom()) ;
     }
    }
   }
  }
...
Double_t TPart_ident_ct::invariant_mass_mctk() {

  TVector3 V3_1 , V3_2; 

  V3_1.SetXYZ(px_mctk(-211),py_mctk(-211),pz_mctk(-211)); // pi- 3-momentum
  V3_2.SetXYZ(px_mctk(211),py_mctk(211),pz_mctk(211)); // pi+ 3-momentum
...
Double_t TPart_ident_ct::Q2_mctk() {

  TVector3 V3_1 , V3_2 ,V3_3;
  V3_1.SetXYZ(px_mctk(11),py_mctk(11),pz_mctk(11));
  V3_2.SetXYZ(0,0,Ebeam);
  V3_3 = V3_2 - V3_1;   // Virtual photon momentum vector
...
Double_t TPart_ident_ct::moment_mctk( Int_t k) {
  p_moment=sqrt(px_mctk(k)*px_mctk(k)+py_mctk(k)*py_mctk(k)+pz_mctk(k)*pz_mctk(k));
  return p_moment;
}
//-----------------------------------

Double_t TPart_ident_ct::nu_mctk() {
  p_nu=Ebeam-sqrt(0.000511*0.000511 +pow(moment_mctk(11),2));
  return p_nu;
}
//-----------------------------------------


Double_t TPart_ident_ct::W_mctk() {
  p_W=sqrt(0.938*0.938+2*0.938*nu_mctk()-Q2_mctk());
  return p_W;
}
//------------------------------------


Double_t TPart_ident_ct::t_mctk() {
  ...
  TLorentzVector V4_1(V3_3,(Ebeam - V3_1.Mag())); // Virtual photon quadrimomentum
  V3_1.SetXYZ(px_mctk(-211),py_mctk(-211),pz_mctk(-211));
  V3_2.SetXYZ(px_mctk(211),py_mctk(211),pz_mctk(211));
...
Double_t TPart_ident_ct::find_rho_energy_mctk() {
  
  rho_energy = sqrt(M_pi*M_pi + pow(moment_mctk(211),2)) + sqrt(M_pi*M_pi + pow(moment_mctk(-211),2)) ;
  return rho_energy;
}
...
Double_t TPart_ident_ct::find_Delta_E_mctk() {

  // k = electron     m = pi-    j = pi+  
  Delta_E = nu_mctk() - find_rho_energy_mctk()  - t_mctk()/(2*0.938272);
  Delta_E = fabs(Delta_E);
  return Delta_E;
}
...
Double_t TPart_ident_ct::find_zrho_mctk() {

  // k = electron     m = pi-    j = pi+  
  zrho  = find_rho_energy_mctk() / nu_mctk();
  return zrho;
}
//-----------------------------------------
...
Double_t TPart_ident_ct::rho_mom_mctk() {
  p_mom_rho_mc = sqrt((px_mctk(211)+px_mctk(-211))*(px_mctk(211)+px_mctk(-211))+(py_mctk(211)+py_mctk(-211))*(py_mctk(211)+py_mctk(-211))+(pz_mctk(-211)+pz_mctk(211))*(pz_mctk(-211)+pz_mctk(211)));
  return p_mom_rho_mc;
...
// Double_t TPart_ident_ct::theta_pip_mctk() {
// 
//   p_theta_pip_mc = 0;
// 
//   TVector3 V3_1 , V3_2 , V3_3, V3_4;
...
//   V3_1.SetXYZ(px_mctk(-211),py_mctk(-211),pz_mctk(-211)); // pi- 3-momentum
//   V3_2.SetXYZ(px_mctk(211),py_mctk(211),pz_mctk(211)); // pi+ 3-momentum
//   V3_4.SetXYZ(px_mctk(2212),py_mctk(2212),pz_mctk(2212)); // outgoing proton 3-momentum
// 
//   TLorentzVector V4_1(V3_1,sqrt(V3_1.Mag2() + pow(0.13957,2))); // pi- quadrimomentum
//   TLorentzVector V4_2(V3_2,sqrt(V3_2.Mag2() + pow(0.13957,2))); // pi+ quadrimomentum
//   TLorentzVector V4_4(V3_4,sqrt(V3_4.Mag2() + pow(0.938272,2))); // outgoing proton quadrimomentum
...
Double_t TPart_ident_ct::theta_pip_mctk() {
  ...
  V3_e.SetXYZ(0,0,Ebeam); // e beam momentum
  V3_e2.SetXYZ(px_mctk(11),py_mctk(11),pz_mctk(11)); // Scattered electron  
  V3_1.SetXYZ(px_mctk(-211),py_mctk(-211),pz_mctk(-211)); // pi- 3-momentum
  V3_2.SetXYZ(px_mctk(211),py_mctk(211),pz_mctk(211)); // pi+ 3-momentum
  V3_4 = ( V3_e - V3_e2 ) - ( V3_1 + V3_2) ; // outgoing proton
					     // 3-momentum = - t =
...
Double_t TPart_ident_ct::phi_lep_had_mctk() {
  ...
  TVector3 V3_1 , V3_2 ,V3_3 , V3_4, V3_5, V3_6;
  V3_1.SetXYZ(px_mctk(11),py_mctk(11),pz_mctk(11)); // e' momentum
  V3_2.SetXYZ(0,0,Ebeam); // e momentum
  V3_3 = V3_2 - V3_1;   // Virtual photon momentum vector
  V3_4.SetXYZ(px_mctk(211),py_mctk(211),pz_mctk(211)); // pi+ momentum
  V3_5.SetXYZ(px_mctk(-211),py_mctk(-211),pz_mctk(-211)); // pi- momentum
  V3_6 = V3_4 + V3_5; // rho0 momentum
...
Double_t TPart_ident_ct::phi_pip_mctk() {
...
Double_t TPart_ident_ct::theta_lam_mctk() {
...
Double_t TPart_ident_ct::phi_lam_mctk() {
