...
//
  ...
  Hpid->Add(proton_vx = new TH1F("proton_vx","Proton vertex x; cm",100,-20.,20.));
  Hpid->Add(proton_vy = new TH1F("proton_vy","Proton vertex y; cm",100,-20.,20.));
  Hpid->Add(proton_vz = new TH1F("proton_vz","Proton vertex z; cm",100,-40.,0.));
  Hpid->Add(miss_mass_1 = new TH1F("miss_mass_1","Missing Mass 1 proton knockout; GeV",200,191.3,193.4));
  Hpid->Add(miss_mass_2 = new TH1F("miss_mass_2","Missing Mass 2 proton knockout; GeV",200,190.3,192.4));
  Hpid->Add(miss_mass_3 = new TH1F("miss_mass_3","Missing Mass 3 proton knockout; GeV",200,189.4,191.5));
  Hpid->Add(miss_mass_4 = new TH1F("miss_mass_4","Missing Mass 4 proton knockout; GeV",200,188.5,190.6));
  Hpid->Add(miss_mass_5 = new TH1F("miss_mass_5","Missing Mass 5 proton knockout; GeV",200,187.5,189.6));
  Hpid->Add(miss_mass_6 = new TH1F("miss_mass_6","Missing Mass 6 proton knockout; GeV",200,186.6,188.7));
  Hpid->Add(miss_mass_7 = new TH1F("miss_mass_7","Missing Mass 7 proton knockout; GeV",200,185.7,187.8));
  ...
  C_Goodpart    = Cuts.AddCounter("Number of particles cut ok");
...
Int_t TPart_id::Run_pknock(Int_t Max_evt){
  ...
  v4_beam.SetPxPyPzE(0.,0.,Ebeam,pow((pow(0.000511,2)+pow(Ebeam,2)),0.5));
  v4_tg.SetPxPyPzE(0.,0.,0.,193.0438); // Target = Lead208 at rest : composition isotopes : 208.*0.524+0.221*207.+206.*0.241+0.014*204
  ...
  for(iEvent=0;iEvent<GetEntries() && Next()==0; iEvent++){
    ...
    if (good_event_towrite == 1) {
      //     filevt->Fill();  // Fill the output chain.
      v4_e.SetPxPyPzE(px(0),py(0),pz(0),pow(pow(0.000511,2)+pow(moment(0),2),0.5));
      v4_p.SetPxPyPzE(0.,0.,0.,0.);
      for (int j=0 ; j<proton_id_pass; j++) { // last one by construction is the scattered electron
	v4.SetPxPyPzE(px(proton_id_pos[j]),py(proton_id_pos[j]),pz(proton_id_pos[j]),pow(pow(0.93827,2)+pow(moment(proton_id_pos[j]),2),0.5));
	v4_p = v4_p + v4;
	proton_vx->Fill(x(proton_id_pos[j]));
	proton_vy->Fill(y(proton_id_pos[j]));
	proton_vz->Fill(z(proton_id_pos[j]));
      }
      v4_t = v4_beam + v4_tg - v4_e - v4_p;
      if      (proton_id_pass == 1 && GetNPart() == 2 && electron_id_pass ==1) {
	miss_mass_1->Fill(v4_t.M());
	filevt->Fill();  // Fill the output chain.
      }
      else if (proton_id_pass == 2 && GetNPart() == 3 && electron_id_pass ==1) {
	miss_mass_2->Fill(v4_t.M());
	filevt->Fill();  // Fill the output chain.
      }
      else if (proton_id_pass == 3 && GetNPart() == 4 && electron_id_pass ==1) {
	miss_mass_3->Fill(v4_t.M());
	filevt->Fill();  // Fill the output chain.
      }
      else if (proton_id_pass == 4 && GetNPart() == 5 && electron_id_pass ==1) {
	miss_mass_4->Fill(v4_t.M());
	filevt->Fill();  // Fill the output chain.
      }
      else if (proton_id_pass == 5 && GetNPart() == 6 && electron_id_pass ==1) {
	miss_mass_5->Fill(v4_t.M());
	filevt->Fill();  // Fill the output chain.
      } 
      else if (proton_id_pass == 6 && GetNPart() == 7 && electron_id_pass ==1) {
	miss_mass_6->Fill(v4_t.M());
	filevt->Fill();  // Fill the output chain.
      }
      else if (proton_id_pass == 7 && GetNPart() == 8 && electron_id_pass ==1) {
	miss_mass_7->Fill(v4_t.M());
	filevt->Fill();  // Fill the output chain.
      }
      else if (proton_id_pass == 8 && GetNPart() == 9 && electron_id_pass ==1) {
	miss_mass_8->Fill(v4_t.M());
	filevt->Fill();  // Fill the output chain.
      }
      else if (proton_id_pass == 9 && GetNPart() == 10 && electron_id_pass ==1) {
	miss_mass_9->Fill(v4_t.M());
	filevt->Fill();  // Fill the output chain.
      }
      else if (proton_id_pass == 10 && GetNPart() == 11 && electron_id_pass ==1) {
	miss_mass_10->Fill(v4_t.M());
	filevt->Fill();  // Fill the output chain.
      }
...
Int_t TPart_id::Run_momcorr(Int_t Max_evt){
  ...
  for(iEvent=0;iEvent<GetEntries() && Next()==0; iEvent++){
	...
	proton_id_pass++;
      ...
      if(Pim_id(ipart)==1) { // Run with Lamiaa PID
	pim_id_pos[pim_id_pass] = ipart;
	//	printf("pion id \t");
	pim_id_pass++;
      }
    ...
    //    printf("finished test on number of particles \t"); 
    ...
    if (good_event_towrite == 1) {
      ...
      v4_e.SetPxPyPzE(px(0),py(0),pz(0),pow(pow(0.000511,2)+pow(moment(0),2),0.5));
      v4_p.SetPxPyPzE(0.,0.,0.,0.);
      v4_pim.SetPxPyPzE(px(pim_id_pos[0]),py(pim_id_pos[0]),pz(pim_id_pos[0]),pow(pow(0.139570,2)+pow(moment(pim_id_pos[0]),2),0.5));
      for (int j=0 ; j<proton_id_pass; j++) { // last one by construction is the scattered electron
	v4.SetPxPyPzE(px(proton_id_pos[j]),py(proton_id_pos[j]),pz(proton_id_pos[j]),pow(pow(0.93827,2)+pow(moment(proton_id_pos[j]),2),0.5));
	v4_p = v4_p + v4;
	proton_vx->Fill(x(proton_id_pos[j]));
