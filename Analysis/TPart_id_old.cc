...
Int_t TPart_id::Acceptance_data(Int_t i){
  ...
  Int_t ok=0;
  Int_t bin_acc=0;
  Double_t Q2_v, W_v, t_v, rho_mom_v, theta_pip_v, phi_pip_v, phi_lep_had_v;
  Q2_v = Q2_mc();
  W_v = W_mc();
  t_v = t_mc();
  ...
  if (Q2_v >0.7 && Q2_v <2 && W_v>1.8 && W_v<3 && t_v>0.1 && t_v<0.4 && rho_mom_v>1.5 && rho_mom_v<4.5 && theta_pip_v>0 &&  theta_pip_v< (TMath::Pi()) && phi_pip_v>0 &&  phi_pip_v<(2*TMath::Pi()) && phi_lep_had_v>0 && phi_lep_had_v< (TMath::Pi()) ) {
    ...
    if(Q2_v >0.7 && Q2_v <1.15) ox=0; //deciding the binning
    if(Q2_v >1.15 && Q2_v <1.4) ox=1;
    if(Q2_v >1.4 && Q2_v <2)    ox=2;

    if( W_v>1.8 && W_v<2.2) oy=0;
    if( W_v>2.2 && W_v<2.6) oy=1;
    if( W_v>2.6 && W_v<3)   oy=2;

    if(t_v>0.1 && t_v<0.25) oz=0;
    if(t_v>0.25 && t_v<0.4) oz=1;

    if( rho_mom_v>1.5 && rho_mom_v<2)   ot=0;
    if( rho_mom_v>2 && rho_mom_v<2.5)   ot=1;
    if( rho_mom_v>2.5 && rho_mom_v<3.5) ot=2;
    if( rho_mom_v>3.5 && rho_mom_v<4.5) ot=3;

    if(theta_pip_v>0 &&  theta_pip_v< (TMath::Pi() /6) )                       oi=0;
    if(theta_pip_v> (TMath::Pi() /6) &&  theta_pip_v< (TMath::Pi() /6 *2) )    oi=1;
    if(theta_pip_v> (TMath::Pi() /6 *2) &&  theta_pip_v< (TMath::Pi() /6 *3) ) oi=2;
    if(theta_pip_v> (TMath::Pi() /6 *3) &&  theta_pip_v< (TMath::Pi() /6 *4) ) oi=3;
    if(theta_pip_v> (TMath::Pi() /6 *4) &&  theta_pip_v< (TMath::Pi() /6 *5) ) oi=4;
    if(theta_pip_v> (TMath::Pi() /6 *5) &&  theta_pip_v< (TMath::Pi()) )       oi=5;

    if(phi_pip_v>0 &&  phi_pip_v< (TMath::Pi() /3) )                       oj=0;
    if(phi_pip_v> (TMath::Pi() /3) &&  phi_pip_v< (TMath::Pi() /3 *2) )    oj=1;
    if(phi_pip_v> (TMath::Pi() /3 *2) &&  phi_pip_v< (TMath::Pi() /3 *3) ) oj=2;
    if(phi_pip_v> (TMath::Pi() /3 *3) &&  phi_pip_v< (TMath::Pi() /3 *4) ) oj=3;
    if(phi_pip_v> (TMath::Pi() /3 *4) &&  phi_pip_v< (TMath::Pi() /3 *5) ) oj=4;
    if(phi_pip_v> (TMath::Pi() /3 *5) &&  phi_pip_v< (TMath::Pi()) *2)     oj=5;

    if(phi_lep_had_v>0 &&  phi_lep_had_v< (TMath::Pi() /3))                       ok=0;
    if(phi_lep_had_v> (TMath::Pi() /3) &&  phi_lep_had_v< (TMath::Pi() /3 *2))    ok=1;
    if(phi_lep_had_v> (TMath::Pi() /3 *2) &&  phi_lep_had_v< (TMath::Pi() /3 *3)) ok=2;    
