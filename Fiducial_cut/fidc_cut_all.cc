...
TFidCut::TFidCut(Char_t* FileName, Int_t part){ // part = 0 --> e- and pi+  ; part=1 --> pi-
  ...
  while( !inputfile.eof() ){
    ...
    cout << "Sector" << sector << " Side" << side << " p" << paraf << " " << p0 << " " << p1<< " " << p2<< " " << p3<< " " << p4<< " " << p5 << endl;
  ...
  for(sector = 1 ; sector < 7 ; sector++) {
	...
	sprintf(Title,"fidc_fun_se%i_si%i_p_%i_t%i",sector,side,paraf,part);
	if (paraf==0 && part==0) {
...
Int_t TFidCut::CheckCut(TVector3 Vector){
  ...
  if ( p_theta > theta_low && p_theta < 54 && mom_phi > fcfidc_fun[0]->Eval(p_theta) && mom_phi < fcfidc_fun[1]->Eval(p_theta) ) {
...
Int_t TFidCut::CheckCutPim(TVector3 Vector){
...
Double_t TFidCut::Get_Par(Int_t sec, Int_t side, Int_t param, Double_t mom){
  Double_t par_mom;
  par_mom = fcfun[sec][side][param]->Eval(mom);
  return par_mom;
